...
-> proxy($proxy);
$soap->autotype(0);
$method = SOAP::Data->name('ValidateOrder')
...
unless ($result->fault) {
	$response = $result->valueof('//ValidateOrderResponse/ValidateOrderResult');
	print "ResultCode : $response->{'ResultCode'}\r\n";
	print "Message : $response->{'Message'}\r\n";
	print "Values: ", join(', ', $result->valueof('//ValidateOrderResponse/ValidateOrderResult/Values/[>0]')), "\n";
}
else {
	print join ', ',
	$result->faultcode,
